...
#     scripts/session_check.sh c_flags_necessary      # every per-TU flag key moves a byte
...
DEFAULT_CHECKS=(splat rel-splat metadata build rel expected expected_stale dual_compiler_regress c_flags_necessary units score diff data_decls forced_regs inline_asm_tree naming_debt naming_sync monolith_sync jtbl_words carve_allowlist compile_units_tracked section_owner registers tu_complete atlas social)
...
# tracked check.
OPTIONAL_CHECKS=(naming_debt naming_sync atlas expected_stale monolith_sync jtbl_words)
