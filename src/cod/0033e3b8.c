...
__attribute__((section(".text.func_0033E3B8")))
void func_0033E3B8(ADXB adxb)
{
    AdxDecPara *dp;
    Sint16 *pcmbuf_l;
    Sint16 *pcmbuf_r;
    ...
    if (adxb->stat == 1) {
        if (Obj33F8_GetWord_C_FA68(adxb->xpd) == 0) {
            ...
            if (adxb->nch == 2) {
                ...
                for (i = 0; i < ndata; i++) {
                    pcmbuf_l[i] = ibuf[i * 4] | (ibuf[i * 4 + 2] << 8);
                    pcmbuf_r[i] = ibuf[i * 4 + 1] | (ibuf[i * 4 + 3] << 8);
                }
            } else {
                for (i = 0; i < ndata; i++) {
                    pcmbuf_l[i] = ibuf[i * 2] | (ibuf[i * 2 + 1] << 8);
                }
            }
            adxb->stat = 2;
            adxb->ndecsmpl = ndata;
