...
/* wave 31 L1_libstdcxx: every inline member of filebuf is DECLARED, not defined, so the TU
   emits no plain-.text global beside the carve (the class vtable lives here, which makes g++ 2.x
   export all of them as strong definitions). --- */
...
extern "C" int _IO_do_write(_IO_FILE *, const char *, _IO_size_t);

__attribute__((section(".text._$_7filebuf")))
filebuf::~filebuf()
{
  if (_fileno >= 0)
    {
      _IO_do_write(this, _IO_write_base, _IO_write_ptr - _IO_write_base);
      if (!(xflags() & _IO_DELETE_DONT_CLOSE))
	(*(int (**)(_IO_FILE *))((char *)(void *)_vtable() + 0x84))(this);
    }
}
