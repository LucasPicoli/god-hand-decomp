...
/* libio genops.c :: _IO_default_setbuf */
__attribute__((section(".text._IO_default_setbuf")))
struct _IO_FILE *_IO_default_setbuf(struct _IO_FILE *fp, char *p, _IO_ssize_t len) {
    if ((*fp->vtable->__sync.pfn)(fp) == -1)
        return 0;
    if (p == 0 || len == 0) {
        fp->_flags |= 2;
