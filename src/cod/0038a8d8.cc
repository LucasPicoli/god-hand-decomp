...
/* ---- body ---- */
extern "C" {
_IO_size_t _IO_getline_info __P ((_IO_FILE *, char *, _IO_size_t, int, int, int *));
/* streambuf::sungetc lives at 0x0038B480; this repo names that address
   Tramp__IO_sungetc_38B480. */
int Tramp__IO_sungetc_38B480(streambuf *);
}
#define sungetc_of(sb) Tramp__IO_sungetc_38B480(sb)

__attribute__((section(".text.getline__7istreamPcic")))
istream& istream::getline(char* buf, int len, char delim)
{
  ...
  if (ipfx1())
	...
	{
	  set(ios::failbit);
	  sungetc_of(sb); // Leave delimiter unread.
	}
      isfx();
