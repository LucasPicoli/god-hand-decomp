...
/* ---- body ---- */
/* `static` upstream.  It is dropped so the symbol binds GLOBAL and the other
   translation units in this block can link to it. */
...
/* libioP.h's _IO_sputn, written out: JUMP2(__xsputn, ...) is a call through the
   _IO_jump_t slot with NO C++ this-adjustment. */
typedef _IO_size_t (*LB2_xsputn_t) (_IO_FILE *, const void *, _IO_size_t);
struct LB2_jt_ent { short d1; short d2; LB2_xsputn_t pfn; };
struct LB2_jumps { char pad[0x30]; struct LB2_jt_ent __xsputn; };
struct _IO_FILE_plus { char file[0x50]; struct LB2_jumps *vtable; };
#define _IO_JUMPS(THIS) ((struct _IO_FILE_plus *) (THIS))->vtable
#define _IO_sputn(FP, DATA, N) _IO_JUMPS(FP)->__xsputn.pfn (FP, DATA, N)

/* Retail holds these literals once in the split .rodata blob. Reference
   the blob so this TU emits an EMPTY .rodata. */
extern "C" {
extern const char D_0045D0F8[];
extern const char D_0045D100[];
extern const char D_0045D118[];
}

__attribute__((section(".text.write_int__FR7ostreamUxi")))
void write_int(ostream& stream, unsigned LONGEST val, int sign)
{
#define WRITE_BUF_SIZE (10 + sizeof(unsigned LONGEST) * 3)
    char buf[WRITE_BUF_SIZE];
    register char *buf_ptr = buf+WRITE_BUF_SIZE; // End of buf.
    const char *show_base = D_0045D0F8;
    int show_base_len = 0;
	...
	const char *xdigs = (stream.flags() & ios::uppercase) ? D_0045D100
	    : D_0045D118;
	do {
