...
/* libio genops.c :: _IO_default_doallocate */
__attribute__((section(".text._IO_default_doallocate")))
int _IO_default_doallocate(struct _IO_FILE *fp) {
    char *buf;

    do {
        buf = (char*)func_003A7C70(1024);
        if (buf == 0)
            return -1;
    } while (0);
    func_0038C768(fp, buf, buf+1024, 1);
    return 1;
}
