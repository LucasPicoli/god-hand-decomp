...
extern Sint32 ADX_DecodeInfoAiff(Sint8 *ibuf, Sint32 ibuflen, Sint16 *dlen, Sint8 *code,
                            Sint8 *bps, Sint8 *blksize, Sint8 *nch, Sint32 *sfreq,
                            Sint32 *total_nsmpl, Sint32 *nsmpl_blk);

__attribute__((section(".text.func_00341008")))
Sint32 func_00341008(ADXB adxb, Sint8 *ibuf, Sint32 ibuflen)
{
    Sint16 dlen;

    adxb->hdcdflag = 1;
    if (ADX_DecodeInfoAiff(ibuf, ibuflen, &dlen, &adxb->code, &adxb->bps, &adxb->blklen,
                      &adxb->nch, &adxb->sfreq, &adxb->total_nsmpl, &adxb->blknsmpl) < 0) {
        return 0;
    }
    adxb->cof = 0;
    adxb->lp_eofst = 0;
