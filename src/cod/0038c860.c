...
/* libio genops.c :: _IO_default_uflow */
__attribute__((section(".text._IO_default_uflow")))
int _IO_default_uflow(struct _IO_FILE *fp) {
    int ch = (*fp->vtable->__underflow.pfn)(fp);
    if (ch == -1)
        return -1;
