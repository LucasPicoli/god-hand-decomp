...
extern Sint16 D_003EC460[256];   /* static ulaw_exp_table in adx_bau.c */

__attribute__((section(".text.func_00341D18")))
void func_00341D18(ADXB adxb)
{
    AdxDecPara *dp;
    Uint16 *pcmbuf_l;
    Uint16 *pcmbuf_r;
    ...
    if (adxb->stat == 1) {
        if (Obj33F8_GetWord_C_FA68(adxb->xpd) == 0) {
            ...
            pcmbuf_l = (Uint16 *)dp->pcmbuf + dp->wpos;
            if (adxb->nch == 2) {
                pcmbuf_r = (Uint16 *)dp->pcmbuf + (dp->pcmbdist + dp->wpos);
                for (i = 0; i < ndata; i++) {
                    pcmbuf_l[i] = D_003EC460[ibuf[i * 2]];
                    pcmbuf_r[i] = D_003EC460[ibuf[i * 2 + 1]];
                }
            } else {
                for (i = 0; i < ndata; i++) {
                    pcmbuf_l[i] = D_003EC460[ibuf[i]];
                }
            }
            adxb->stat = 2;
            adxb->ndecsmpl = ndata;
