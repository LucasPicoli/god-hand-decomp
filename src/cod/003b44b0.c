...
extern char D_00460138[];
extern char D_00780C00[];
extern char D_007816C0[];

__attribute__((section(".text.func_003B44B0")))
int func_003B44B0(int fd)
{
    struct SemaParam sp;
    int result;
    Iob *p;
    ...
    sp.maxCount = 1;
    sp.initCount = 0;
    sp.option = (unsigned int)D_00460138;
    sid = CreateSema(&sp);
    D_0077FFC0.sema = sid;
    q->dst = &result;
    q->size = 4;
    r = func_003B3958(D_007816C0, 1, 0, q, 0x14, D_00780C00, 4, 0, 0, 0);
    if (r < 0) {
        DeleteSema(sid);
        SignalSema_3B3EF8();
        return -11;
